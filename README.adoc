...
TODO: this currently only works for modules built with Buildroot, see: <<kernel_modules-package>>.

We could likely make it work with link:build-modules[] without much effort by copying whatever Buildroot is doing.
...
This is the default install path for `make modules_install` in the Linux kernel, with root path given by `INSTALL_MOD_PATH`, and therefore canonical in that sense.

Kernel modules built with `CONFIG_SOME_MOD=m` are installed in that location automatically available with `modprobe`, e.g.:
...
TODO: `modprobe` currently fails with wrong module format because we need to boot the Buildroot Linux kernel for that to work. TODO expose that with a `./run --buildroot-kernel` option. Lazy now.

Source: link:buildroot_packages/kernel_modules/[]

An example of how to kernel modules in Buildroot.

Described in detail at: https://stackoverflow.com/questions/40307328/how-to-add-a-linux-kernel-driver-module-as-a-buildroot-package/43874273#43874273
...
Then use the modules with:

....
modprobe hello
....

`--build-linux` is needed because the kernel modules built with Buildroot also need the kernel built with Buildroot.
